...
namespace DynamicsApiToDatabase.Services
{
    public class DynamicsDataService
    {
        ...
        /// <summary>
        /// Logique HASH EXISTANTE pour les autres endpoints
        /// MODIFIÉ : Confirmation spéciale pour INT32
        /// </summary>
        ...
        {
            // Filtrage des éléments déjà traités (logique existante)
        ...
        private string GenerateUniqueKey(JsonElement item, string primaryKeyField, string endpointName)
        {
            try
            {
                switch (endpointName.ToUpper())
                {
                    ...
                    // ✅ MODIFIÉ : Return Orders - Retirer suffixes -2, -3, etc.
                    case "RETURNORDERS":
                    case "BRINT32RETURNORDERTABLES":
                    ...
                    // ✅ MODIFIÉ : Purchase Orders - Retirer suffixes -2, -3, etc.
                    case "PURCHASEORDERS":
                    case "BRINT32PURCHORDERTABLES":
                    ...
                    // ✅ MODIFIÉ : Transfer Orders - Retirer suffixes -2, -3, etc.
                    case "TRANSFERORDERS":
                    case "BRINT32TRANSFERORDERTABLES":
        ...
        /// MODIFIÉ : Pour INT32, traite toutes les lignes individuellement
        /// </summary>
        private async Task<List<JsonElement>> FilterUnprocessedOrdersAsync(List<JsonElement> apiData, string orderIdField, string endpointPath)
        {
            ...
            // ✅ Récupérer les IDs déjà traités (vide pour INT32)
            ...
            // ✅ NOUVEAU : Pour INT32, vérifier ligne par ligne si elle existe en BDD
            bool isInt32 = endpointPath.Contains("BRINT32");

            if (isInt32)
            {
                _logger.LogInformation($"🔄 [INT32] Mode ligne par ligne pour {endpointPath}");

                // Pour INT32 : traiter toutes les lignes de l'API
                // Le filtrage se fera par hash dans InsertOrUpdateJsonDataAsync
                return apiData;
            }
            else
            {
